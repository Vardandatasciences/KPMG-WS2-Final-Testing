...
class ChangeManagementService:
    ...
    def _extract_pdf_text_snippet(self, file_path: Path, max_pages: int = 10, max_chars: int = 12000) -> Optional[str]:
    ...
    def analyze_pdf_with_ai(self, file_path: Path, framework_hint: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if not self.ai_enabled:
            logger.info("AI analysis disabled for %s - API key not configured", file_path.name)
            return None
        
        if not self.openai_client and not self.legacy_openai:
            logger.warning("AI analysis unavailable for %s - OpenAI client not initialized", file_path.name)
            return None
        
        snippet = self._extract_pdf_text_snippet(file_path)
        if not snippet:
            logger.info("AI analysis skipped for %s - no text extracted.", file_path.name)
            print(f"⚠️ Could not extract text from PDF for AI analysis")
            return None
        
        print(f"📝 Extracted {len(snippet)} characters from PDF for AI analysis")
        
        user_prompt_text = (
            "You are analyzing a document about changes or updates in a security, compliance, or regulatory framework. "
            "This could be a 'Summary of Changes', version comparison, amendment document, or update notice.\n\n"
            
            "**Your Task:**\n"
            "Carefully read the document and extract ALL structural information about what changed, including:\n\n"
            
            "1. **Framework Identification:**\n"
            "   - Identify the framework name (e.g., NIST SP 800-53, PCI DSS, ISO 27001, HIPAA, GDPR, SOX, Basel III)\n"
            "   - Look for version numbers (e.g., 'v4.0', 'Release 5.2.0', 'Rev 1')\n"
            "   - Note any alternative names or abbreviations used\n\n"
            
            "2. **Modified Requirements/Controls/Policies:**\n"
            "   - Extract EACH modified requirement, control, or policy mentioned\n"
            "   - Include the requirement/control identifier (e.g., 'Requirement 1.2.3', 'AC-2', 'Control A.5.1')\n"
            "   - Include the full name of the requirement/control\n"
            "   - Describe what specifically changed (e.g., 'wording clarified', 'new requirements added', 'scope expanded')\n"
            "   - List any enhancements or updates made\n"
            "   - If sub-requirements or sub-policies are mentioned, include them\n\n"
            
            "3. **New Additions:**\n"
            "   - Identify ALL newly added requirements, controls, policies, or sections\n"
            "   - Include identifiers, names, scope, and purpose\n"
            "   - Note why they were added if mentioned\n\n"
            
            "4. **Deprecated/Removed Items:**\n"
            "   - Note any items that were removed, deprecated, or retired\n\n"
            
            "5. **Cross-References:**\n"
            "   - Identify references to other frameworks or standards\n"
            "   - Note any mappings, alignments, or compliance relationships\n\n"
            ...
            "- Look for tables, lists, or sections that enumerate changes\n"
            "- Pay attention to requirement/control IDs, section numbers, or policy names\n"
            "- For PCI DSS: look for 'Requirement X.X.X' patterns\n"
            "- For NIST: look for control IDs like 'AC-2', 'SI-7', etc.\n"
            "- For ISO: look for 'A.X.X' or clause numbers\n\n"
            
            "**Output Format:**\n"
            "Respond ONLY with valid JSON (no markdown, no explanations). Use this exact structure:\n\n"
            "{\n"
            "  \"framework_name\": \"<Full framework name with version if available>\",\n"
            "  \"probable_aliases\": [\"<Alternative names or abbreviations>\"],\n"
            "  \"confidence\": <0.0-1.0 confidence score>,\n"
            "  \"summary\": \"<2-3 sentence overview of what changed in this document>\",\n"
            "  \"modified_controls\": [\n"
            "    {\n"
            "      \"control_id\": \"<Identifier like 'Requirement 1.2.3' or 'AC-2'>\",\n"
            "      \"control_name\": \"<Full name/title of the control>\",\n"
            "      \"change_type\": \"<modified | enhanced | clarified | restructured>\",\n"
            "      \"change_description\": \"<Specific description of what changed>\",\n"
            "      \"enhancements\": [\"<List of specific enhancements>\"],\n"
            "      \"related_controls\": [\"<Related control IDs if mentioned>\"],\n"
            "      \"sub_policies\": [\n"
            "        {\n"
            "          \"sub_policy_name\": \"<Sub-requirement or sub-control name>\",\n"
            ...
            "  \"new_additions\": [\n"
            "    {\n"
            "      \"control_id\": \"<New control/requirement identifier>\",\n"
            "      \"control_name\": \"<Name of new control/requirement>\",\n"
            ...
            "    {\n"
            "      \"referenced_framework\": \"<Name of referenced framework/standard>\",\n"
            "      \"reference_type\": \"<mapping | alignment | compliance | supersedes>\",\n"
            "      \"description\": \"<How it relates>\"\n"
            "    }\n"
            "  ]\n"
            "}\n\n"
            
            "**Critical:** If the document lists specific changes (e.g., 'Requirement 1.2.3 was updated'), "
            "you MUST extract each one individually. Do NOT return empty arrays if changes are mentioned. "
            "Extract as much detail as possible from the text provided."
        )
        if framework_hint:
        ...
        try:
            ...
            if self.openai_client:
                if hasattr(self.openai_client, "responses"):
                    try:
                        ...
                        logger.error("OpenAI responses API call failed: %s", str(e))
                        print(f"❌ OpenAI API error (responses): {str(e)}")
                        return None
                    ...
                    try:
                        ...
                        logger.error("OpenAI chat API call failed: %s", str(e))
                        print(f"❌ OpenAI API error (chat): {str(e)}")
                        return None
            ...
            if not ai_text and self.legacy_openai:
                try:
                    ...
                    logger.error("Legacy OpenAI API call failed: %s", str(e))
                    print(f"❌ OpenAI API error (legacy): {str(e)}")
                    return None

            if not ai_text:
                logger.warning("OpenAI response did not contain text for %s", file_path.name)
                print(f"⚠️ AI analysis failed: No response text received")
                return None

            ai_text = ai_text.strip()
            original_response = ai_text  # Keep original for debugging
            ...
            try:
                ...
                logger.info("Successfully parsed AI analysis for %s", file_path.name)
                print(f"✅ AI analysis successful - extracted structured data")
            except json.JSONDecodeError as e:
                logger.warning("Failed to parse AI JSON for %s. Error: %s", file_path.name, str(e))
                print(f"⚠️ AI analysis JSON parsing failed: {str(e)}")
            ...
            if 'modified_controls' not in analysis or not isinstance(analysis.get('modified_controls'), list):
                analysis['modified_controls'] = []
            
            if 'new_additions' not in analysis or not isinstance(analysis.get('new_additions'), list):
                analysis['new_additions'] = []
            
            if 'framework_references' not in analysis or not isinstance(analysis.get('framework_references'), list):
                analysis['framework_references'] = []
            ...
            print(f"📊 AI extracted: {len(analysis.get('modified_controls', []))} modified controls, "
                  f"{len(analysis.get('new_additions', []))} new additions, "
                  f"{len(analysis.get('framework_references', []))} framework references")
    ...
    def process_pdf_file(self, file_path: Path, user_id: str = "system") -> Dict:
        ...
        try:
            ...
            print(f"🤖 Starting AI analysis..." if self.ai_enabled else "⚠️ AI analysis disabled - using fallback detection")
            ai_analysis = self.analyze_pdf_with_ai(file_path)
            if ai_analysis:
                result['ai_analysis'] = ai_analysis
                print(f"✅ AI analysis completed - Framework: {ai_analysis.get('framework_name', 'Unknown')}")
            else:
                print(f"⚠️ AI analysis returned no results - using filename-based detection")
