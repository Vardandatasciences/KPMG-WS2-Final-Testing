...
def resubmit_compliance_approval(request, approval_id):
    try:
        approval = ComplianceApproval.objects.get(ApprovalId=approval_id)
        ...
        #print(f"DEBUG: Resubmit - Impact: {extracted_data.get('Impact')}, Probability: {extracted_data.get('Probability')}")
        #print(f"DEBUG: Resubmit - All ExtractedData keys: {list(extracted_data.keys()) if extracted_data else 'None'}")
        all_versions = ComplianceApproval.objects.filter(Identifier=approval.Identifier)
        # Keep original creator and reviewer consistent for resubmissions
        # Preserve the same UserId/ReviewerId as the most recent user edit (latest u*)
        latest_user_version = all_versions.filter(Version__startswith='u').order_by('-ApprovalId').first()
        original_user_id = latest_user_version.UserId if latest_user_version else approval.UserId
        original_reviewer_id = latest_user_version.ReviewerId if latest_user_version else approval.ReviewerId
        
        print(f"🔍 DEBUG: Resubmit - Original ReviewerId: {original_reviewer_id} (from {'latest_user_version' if latest_user_version else 'original_approval'})")
        print(f"🔍 DEBUG: Resubmit - Original UserId: {original_user_id}")
        ...
        if 'Impact' not in extracted_data or not extracted_data['Impact']:
            # Try to get from the original approval
            if approval.ExtractedData and approval.ExtractedData.get('Impact'):
                extracted_data['Impact'] = approval.ExtractedData['Impact']
            else:
                # Try to get from the Compliance table
                try:
                    compliance = Compliance.objects.get(Identifier=approval.Identifier)
                    extracted_data['Impact'] = compliance.Impact
                except Compliance.DoesNotExist:
                    pass
        
        if 'Probability' not in extracted_data or not extracted_data['Probability']:
            # Try to get from the original approval
            if approval.ExtractedData and approval.ExtractedData.get('Probability'):
                extracted_data['Probability'] = approval.ExtractedData['Probability']
            else:
                # Try to get from the Compliance table
                try:
                    compliance = Compliance.objects.get(Identifier=approval.Identifier)
                    extracted_data['Probability'] = compliance.Probability
                except Compliance.DoesNotExist:
                    pass
        ...
        print(f"✅ New approval ReviewerId: {new_approval.ReviewerId}, UserId: {new_approval.UserId}, FrameworkId: {new_approval.FrameworkId_id}")
        #print(f"DEBUG: Final approval ExtractedData - Impact: {new_approval.ExtractedData.get('Impact')}, Probability: {new_approval.ExtractedData.get('Probability')}")
        
        # Update the compliance status back to "Under Review" when resubmitting
        try:
            compliance = Compliance.objects.filter(Identifier=approval.Identifier).first()
            if compliance:
                print(f"Updating compliance {compliance.ComplianceId} status from '{compliance.Status}' to 'Under Review'")
                compliance.Status = 'Under Review'
                compliance.ActiveInactive = 'Inactive'
                compliance.save()
                print(f"Successfully updated compliance status to 'Under Review'")
            else:
                print(f"No compliance found for Identifier {approval.Identifier} during resubmission")
        except Exception as e:
            print(f"Error updating compliance status during resubmission: {str(e)}")
...
def edit_compliance(request, compliance_id):
    ...
    try:
        ...
        extracted_data = {
            ...
            'CreatedByName': new_compliance.CreatedByName,  # Add the missing CreatedByName field
        ...
        identifier = new_compliance.Identifier
        all_versions = ComplianceApproval.objects.filter(Identifier=identifier)
        # Find the next user version (uN) for this Identifier
        highest_u_version = 0
        for pa in all_versions:
            if pa.Version and pa.Version.startswith('u'):
