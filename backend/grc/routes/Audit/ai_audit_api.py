...
def analyze_compliance_with_ai(text, doc_name):
    ...
    try:
        ...
        return {
            ...
            'document_type_detected': 'compliance_document' if 'policy' in text_lower else 'general_document'
        }
    except Exception as e:
...
def extract_document_metadata(file_path, doc_type):
...
def generate_ai_recommendations(text, compliance_analysis):
    """Generate AI-powered recommendations"""
    try:
        recommendations = []
        
        if compliance_analysis.get('status') == 'non_compliant':
            recommendations.append("Document lacks sufficient compliance indicators. Consider adding policy references and control measures.")
        
        if compliance_analysis.get('status') == 'partially_compliant':
            recommendations.append("Document shows some compliance elements. Enhance with additional policy frameworks and risk controls.")
        
        if 'security' not in text.lower():
            recommendations.append("Consider adding security-related content to improve compliance posture.")
        
        if 'audit' not in text.lower():
            recommendations.append("Include audit trail information for better compliance tracking.")
        
        if not recommendations:
            recommendations.append("Document appears to meet basic compliance requirements. Continue monitoring and regular updates.")
...
def assess_risk_level(compliance_analysis, metadata):
    """Assess risk level based on compliance analysis"""
    try:
        status = compliance_analysis.get('status', 'unknown')
        confidence = compliance_analysis.get('confidence', 0.0)
        
        if status == 'compliant' and confidence > 0.8:
            return 'low'
        elif status == 'partially_compliant' or (status == 'compliant' and confidence <= 0.8):
            return 'medium'
        else:
            return 'high'
    except Exception as e:
...
def calculate_confidence_score(text, compliance_analysis):
    """Calculate confidence score for AI analysis"""
    try:
        base_confidence = compliance_analysis.get('confidence', 0.0)
...
def extract_audit_document_content(file_path, doc_type):
    """Extract structured content from audit documents"""
    try:
        logger.info(f"📄 Extracting audit document content from: {file_path}")
        
        # Extract basic text content
        full_text = extract_text_from_document(file_path, doc_type)
        
        # Extract generic headers and structure
        headers = extract_document_headers(full_text, doc_type)
        
        # Generic document statistics (policy-agnostic)
        audit_data = extract_audit_data(full_text, doc_type)
        
        # Calculate data quality score (generic)
        quality_score = calculate_data_quality_score(full_text, headers, audit_data)
        ...
        return {
            ...
            'audit_structure': {
                ...
                'data_completeness': len(full_text) / 1000.0
            },
            'quality_score': quality_score
        }
        
    except Exception as e:
        logger.error(f"Error extracting audit document content: {e}")
        return {
            'full_text': '',
            'headers': [],
            'audit_data': {},
...
def perform_audit_compliance_analysis(document_data, doc_name, audit_id):
    """Perform AI-powered audit compliance analysis"""
    try:
        logger.info(f"🔍 Performing audit compliance analysis for: {doc_name}")
        
        full_text = document_data.get('full_text', '')
        audit_structure = document_data.get('audit_structure', {})
        
        # Define compliance requirements
        compliance_requirements = {
            'data_integrity': {
                'required_elements': ['employee_id', 'timestamp', 'action'],
                'weight': 0.3
            },
            'access_controls': {
                'required_elements': ['access_point', 'role', 'permission'],
                'weight': 0.25
            },
            'audit_trail': {
                'required_elements': ['user_id', 'action', 'timestamp', 'result'],
                'weight': 0.25
            },
            'security_policies': {
                'required_elements': ['policy_reference', 'compliance_status'],
                'weight': 0.2
            }
        }
        ...
        for requirement, config in compliance_requirements.items():
            score = 0
            found = []
            missing = []
            
            for element in config['required_elements']:
                if element in full_text.lower() or any(keyword in full_text.lower() for keyword in element.split('_')):
                    score += 1
            ...
            # Normalize score
            normalized_score = score / len(config['required_elements'])
            ...
            found_controls.extend(found)
            missing_controls.extend(missing)
        
        # Calculate overall compliance score
        overall_score = sum(
            compliance_scores[req]['score'] * compliance_scores[req]['weight']
            for req in compliance_scores
        )
        ...
        # Extract specific audit findings
        audit_findings = extract_audit_findings(full_text, compliance_scores)
        
        return {
            'overall_status': status,
            'compliance_score': overall_score,
            'compliance_breakdown': compliance_scores,
            'found_controls': list(set(found_controls)),
            'missing_controls': list(set(missing_controls)),
            'audit_findings': audit_findings,
            'risk_level': risk_level,
            'analysis_timestamp': datetime.now().isoformat(),
            'document_type_detected': 'audit_document'
        }
        
    except Exception as e:
        logger.error(f"Error in audit compliance analysis: {e}")
        return {
...
def determine_audit_compliance_status(audit_analysis):
    """Determine if audit document is COMPLIANT or NON-COMPLIANT"""
    status = audit_analysis.get('overall_status', 'unknown')
...
def calculate_audit_confidence_score(document_data, audit_analysis):
    """Calculate confidence score for audit analysis"""
    try:
        base_confidence = audit_analysis.get('compliance_score', 0.0)
        quality_score = document_data.get('quality_score', 0.0)
        
        # Weight the confidence based on data quality
...
def extract_document_headers(text, doc_type):
    """Extract headers and structure from document"""
    headers = []
    ...
    for line in lines[:20]:  # Check first 20 lines for headers
        ...
        if line and (
            line.isupper() or
            any(keyword in line.lower() for keyword in ['id', 'name', 'date', 'time', 'status', 'type', 'department']) or
            line.count('\t') > 2 or
            line.count(',') > 3
        ):
            headers.append(line)
...
def extract_audit_data(text, doc_type):
    """Extract generic statistics from document (policy-agnostic)."""
    import re
...
def extract_time_range(text):
    """Extract time range from audit data"""
    import re
...
def calculate_data_quality_score(text, headers, audit_data):
    """Calculate data quality score for audit document"""
    score = 0.0
    ...
    # Basic completeness heuristics (generic)
    if audit_data.get('line_count', 0) > 20:
        score += 0.2
    if audit_data.get('unique_word_count', 0) > 100:
        score += 0.2
    
    return min(score, 1.0)


def extract_audit_findings(text, compliance_scores):
    """Placeholder: build generic finding messages from scores."""
    findings = []
    for requirement, data in compliance_scores.items():
        status = 'PASSED' if data['score'] >= 0.8 else 'PARTIAL' if data['score'] >= 0.5 else 'FAILED'
        entry = {
            'requirement': requirement,
            'status': status,
            'recommendation': 'Address missing elements and strengthen evidence' if status != 'PASSED' else 'Maintain current controls'
        }
        findings.append(entry)
    return findings


def extract_audit_metadata(file_path, doc_type, document_data):
    """Extract comprehensive audit metadata"""
    try:
        ...
        return {
            ...
            'file_extension': os.path.splitext(file_path)[1],
            'audit_structure': document_data.get('audit_structure', {}),
            'data_quality': document_data.get('quality_score', 0.0)
        }
    except Exception as e:
        logger.error(f"Error extracting audit metadata: {e}")
        return {}


def generate_audit_recommendations(document_data, audit_analysis):
    """Generate audit recommendations based on compliance analysis"""
    recommendations = []
    
    compliance_score = audit_analysis.get('compliance_score', 0.0)
    missing_controls = audit_analysis.get('missing_controls', [])
    audit_findings = audit_analysis.get('audit_findings', [])
    
    if compliance_score < 0.6:
        recommendations.append("CRITICAL: Document shows significant compliance gaps. Immediate remediation required.")
    
    if 'employee_id' in missing_controls:
        recommendations.append("Add employee identification fields to improve audit trail completeness.")
    
    if 'timestamp' in missing_controls:
        recommendations.append("Include detailed timestamps for all audit events.")
    
    if 'access_point' in missing_controls:
        recommendations.append("Document access points and entry/exit locations.")
    
    if 'policy_reference' in missing_controls:
        recommendations.append("Reference specific policies and standards in audit documentation.")
    
    # Add findings-based recommendations
    for finding in audit_findings:
        if finding['status'] in ['FAILED', 'PARTIAL']:
            recommendations.append(finding['recommendation'])
    
    if not recommendations:
        recommendations.append("Document meets audit compliance requirements. Continue monitoring.")
...
def perform_ai_document_analysis(full_text, doc_name, audit_id):
    ...
    # Advanced document compliance analysis prompt
    prompt = f"""You are an expert GRC compliance auditor. Perform a comprehensive compliance analysis of this document.

DOCUMENT DETAILS:
Name: {doc_name}
Content: {full_text[:1200]}

COMPREHENSIVE COMPLIANCE ANALYSIS TASK:
1. **Overall Compliance Assessment**: Evaluate the document's overall compliance posture
2. **Evidence Quality Analysis**: Assess the quality and completeness of compliance evidence
3. **Risk Assessment**: Determine the risk level based on compliance gaps
4. **Data Quality Evaluation**: Assess the quality and reliability of the data
5. **Document Classification**: Identify the type and purpose of the document
6. **Key Findings Summary**: Extract the most important compliance findings
7. **Gap Analysis**: Identify specific compliance gaps and deficiencies
8. **Recommendations**: Provide actionable recommendations for improvement

COMPLIANCE LEVELS:
- **COMPLIANT**: Strong evidence, comprehensive coverage, well-documented, low risk
- **PARTIALLY_COMPLIANT**: Some evidence, but gaps or weaknesses identified, medium risk
- **NON_COMPLIANT**: Insufficient evidence, major gaps, high risk
...
{{
  ...
  "document_type": "audit_report|policy_document|procedure_manual|training_material|evidence_document",
  "document_purpose": "Brief description of document purpose",
  "compliance_framework": "Applicable compliance framework if identified",
  "key_findings": [
    "Specific compliance elements found in the document",
    "Important evidence or controls identified"
  ],
  "missing_elements": [
  ...
  "strengths": [
    "What the document does well for compliance",
    "Strong evidence or controls present"
  ],
  "weaknesses": [
    "Areas where the document falls short",
    "Compliance gaps or weaknesses"
  ],
  "critical_issues": [
    "High-risk compliance issues that need immediate attention"
  ],
  "recommendations": [
...
ANALYSIS CRITERIA:
- Look for policies, procedures, controls, metrics, documentation standards
- Evaluate evidence completeness, accuracy, and implementation quality
- Consider regulatory context and industry best practices
- Assess both explicit compliance statements and implied compliance through practices
- Identify any red flags or critical compliance issues

CRITICAL: Return ONLY valid JSON. No explanations, no additional text. Use proper JSON syntax with double quotes.

JSON:"""
    
    # Send to OpenAI for analysis
    logger.info(f"🤖 Sending document to OpenAI for analysis: {doc_name}")
    try:
        # Use unified AI API call
        ai_response = call_ai_api(prompt, audit_id, None, 'analysis')
        ...
        try:
            ...
            raise Exception(f"Failed to parse AI analysis JSON: {e}. Response: {json_str[:200]}...")
        
        logger.info(f"✅ AI analysis completed for {doc_name}")
        return ai_analysis
...
def generate_ai_recommendations(full_text, ai_analysis):
    """Generate AI recommendations using OpenAI"""
    import requests
    import json
    
    logger.info("🤖 Using OpenAI for AI recommendations")
    
    # Advanced recommendations prompt
    prompt = f"""You are an expert GRC compliance auditor with extensive experience in regulatory frameworks and compliance management. Based on the comprehensive analysis results, provide detailed, actionable recommendations.

COMPREHENSIVE ANALYSIS RESULTS:
{json.dumps(ai_analysis, indent=2)}

ADVANCED RECOMMENDATIONS TASK:
Provide 5 prioritized, specific, and actionable recommendations that address:

1. **Critical Compliance Gaps**: Address high-risk compliance issues
2. **Process Improvements**: Enhance existing compliance processes
3. **Evidence Strengthening**: Improve compliance documentation and evidence
4. **Risk Mitigation**: Reduce compliance risks and vulnerabilities
5. **Best Practice Implementation**: Align with industry standards and best practices

REQUIRED RECOMMENDATIONS FORMAT:
Return ONLY this structured format (no other text):

**PRIORITY 1 - CRITICAL:** [High-priority recommendation with specific implementation steps, timeline, and responsible parties]

**PRIORITY 2 - HIGH:** [Important recommendation with implementation guidance and expected outcomes]

**PRIORITY 3 - MEDIUM:** [Moderate priority recommendation with specific actions and resources needed]

**PRIORITY 4 - MEDIUM:** [Another moderate priority recommendation with clear next steps]

**PRIORITY 5 - LOW:** [Lower priority but valuable recommendation for continuous improvement]
...
- Focus on measurable results and compliance improvements
- Provide practical guidance that can be implemented by the organization"""
    
    # Use unified AI API call
    recommendations = call_ai_api(prompt, audit_id=None, document_id=None, model_type='recommendations')
    logger.info("✅ AI recommendations generated")
    return recommendations


def extract_ai_metadata(file_path, doc_type, full_text, ai_analysis):
