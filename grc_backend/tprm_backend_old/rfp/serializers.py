...
class RFPSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        """
        Override to handle missing auto_publish column gracefully
        """
        data = super().to_representation(instance)
        # Check if auto_publish column exists in database
        try:
            # Try to access the field - if it doesn't exist, set default
            if 'auto_publish' not in data or data['auto_publish'] is None:
                # Check if field exists on model instance
                if hasattr(instance, 'auto_publish'):
                    data['auto_publish'] = getattr(instance, 'auto_publish', False)
                else:
