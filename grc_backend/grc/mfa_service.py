...
class MfaService:
    ...
    def create_mfa_challenge(cls, user, request=None):
        ...
        # Get IP address and truncate if needed (before encryption)
        ip_address = None
        if request:
            ip_address = MfaAuditLog.get_client_ip(request)
            # Additional safety: truncate to 100 chars max (encryption will expand it)
            if ip_address and len(ip_address) > 100:
