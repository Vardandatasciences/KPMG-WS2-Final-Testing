...
def require_tenant(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        # Try to get tenant from user_id in request data
        # For DRF requests, check request.data first
        user_id = None
        if hasattr(request, 'data') and isinstance(request.data, dict):
            user_id = request.data.get('user_id')
        
        # For file uploads (multipart/form-data), also check request.POST
        if not user_id and request.method == 'POST' and hasattr(request, 'POST'):
            ...
            try:
                ...
                if hasattr(user, 'tenant_id') and user.tenant_id:
                    ...
                    try:
                        ...
                        logger.info(f"[Tenant Utils] Got tenant {user.tenant_id} from user_id {user_id} in request data")
                        return view_func(request, *args, **kwargs)
