        ...
        if encrypted_fields:
            ...
            for field_name in encrypted_fields:
                ...
                if hasattr(self, field_name):
                    try:
                        # Check if field type is encryptable (skip BinaryField, etc.)
                        from .encryption_config import is_field_encryptable
                        if not is_field_encryptable(self.__class__, field_name):
                            logger.debug(f"Skipping encryption of non-encryptable field '{field_name}' ({self.__class__.__name__})")
                            continue
                        ...
                        # Skip bytes/BinaryField - they should not be encrypted
                        if isinstance(field_value, (bytes, bytearray)):
                            logger.debug(f"Skipping encryption of bytes field '{field_name}' for {self.__class__.__name__}")
                            continue
