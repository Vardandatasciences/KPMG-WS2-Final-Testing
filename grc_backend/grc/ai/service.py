...
class AIService:
    ...
    def _prepare_prompt(self, task_name: str, prompt: str) -> dict[str, Any]:
        context = build_context_window(prompt, strategy="balanced", task_complexity="complex" if "gap" in task_name else "medium")
        ...
        *,
        ...
        if task_name.startswith("risk."):
            if isinstance(result, dict):
                ...
                print(f"[AI-RISK] generate_json result dict for task={task_name}, keys={keys}")
            else:
                print(f"[AI-RISK] generate_json result type for task={task_name}: {type(result).__name__}")
