...
class GlobalSearchViewSet(viewsets.ViewSet):
    ...
    def get_filter_options(self, request):
        ...
        try:
            ...
            # Use correct app labels from INSTALLED_APPS
            models_with_filters = {}
            try:
                # Try to get models with correct app labels
                # Vendor model from apps.vendor_core
                try:
                    models_with_filters['vendor'] = apps.get_model('apps.vendor_core', 'Vendors')
                except (LookupError, ValueError):
                    try:
                        models_with_filters['vendor'] = apps.get_model('vendor_core', 'Vendors')
                    except (LookupError, ValueError):
                        pass
                
                # RFP model from rfp app
                try:
                    models_with_filters['rfp'] = apps.get_model('rfp', 'RFP')
                except (LookupError, ValueError):
                    pass
                
                # Contract model from contracts app
                try:
                    models_with_filters['contract'] = apps.get_model('contracts', 'VendorContract')
                except (LookupError, ValueError):
                    pass
                
                # SLA model from slas app
                try:
                    models_with_filters['sla'] = apps.get_model('slas', 'VendorSLA')
                except (LookupError, ValueError):
                    pass
                
                # BCP/DRP models from bcpdrp app
                try:
                    models_with_filters['bcp_drp_plans'] = apps.get_model('bcpdrp', 'Plan')
                except (LookupError, ValueError):
                    pass
                try:
                    models_with_filters['bcp_drp_evaluations'] = apps.get_model('bcpdrp', 'Evaluation')
                except (LookupError, ValueError):
                    pass
