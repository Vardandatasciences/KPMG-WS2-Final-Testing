...
class LlamaService:
    ...
    def create_risks_from_entity_data_row(self, entity: str, table_name: str, row_data: dict) -> List[Risk]:
        ...
        BCP/DRP risk generation is handled by risk_analysis module; this service supports Contract only.
        
        Args:
            entity: Module name (Contract, Vendor, RFP, SLA - not BCP_DRP)
    ...
    def _build_simple_prompt(self, module_data) -> str:
        """Build simple prompt for risk analysis (legacy method - use entity-data-row approach). Contract module only."""
...
Generate specific risks based on the actual data provided:"""
    ...
    def _build_entity_row_prompt(self, entity: str, table_name: str, row_data: dict) -> str:
        """Build prompt for entity-row specific risk analysis. Contract module only; BCP/DRP is handled by risk_analysis."""
        from datetime import date
        today = date.today().strftime('%Y-%m-%d')
