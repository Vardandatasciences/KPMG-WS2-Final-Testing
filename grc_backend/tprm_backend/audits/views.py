...
def review_audit(request, audit_id):
    ...
    try:
        ...
        # Enforce maker–checker: assignee cannot review/approve their own audit
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.warning("[Audit Review] Invalid assignee or reviewer id while reviewing audit")
            return Response(
        ...
        if current_user_id_int is None:
            return Response(
                {'error': 'User authentication required to review audits'},
                status=status.HTTP_401_UNAUTHORIZED
            )

        if action == 'approve' and assignee_id_int is not None and assignee_id_int == current_user_id_int:
            logger = logging.getLogger(__name__)
            logger.warning(f"[Audit Review] Self-approval attempt detected for audit {audit_id} by user {current_user_id_int}")
            return Response(
                {'error': 'Self-approval is not allowed. Please assign a different reviewer for this audit.'},
